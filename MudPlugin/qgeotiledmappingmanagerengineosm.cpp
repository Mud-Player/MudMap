...
:   QGeoTiledMappingManagerEngine()
{
    ...
    if (parameters.contains(QStringLiteral("mud.mapping.cache.directory"))) {
        m_cacheDirectory = parameters.value(QStringLiteral("mud.mapping.cache.directory")).toString();
    } else {
    ...
    if (parameters.contains(QStringLiteral("mud.mapping.providersrepository.address"))) {
        QString customAddress = parameters.value(QStringLiteral("mud.mapping.providersrepository.address")).toString();
    ...
    bool highdpi = false;
    if (parameters.contains(QStringLiteral("mud.mapping.highdpi_tiles"))) {
        const QString param = parameters.value(QStringLiteral("mud.mapping.highdpi_tiles")).toString().toLower();
        if (param == "true")
            highdpi = true;
    }
    ...
    if (parameters.contains(QStringLiteral("mud.mapping.custom.host"))
            || parameters.contains(QStringLiteral("mud.mapping.host"))) {
        // Adding a custom provider
        QString tmsServer;
        if (parameters.contains(QStringLiteral("mud.mapping.host")))
            tmsServer = parameters.value(QStringLiteral("mud.mapping.host")).toString();
        if (parameters.contains(QStringLiteral("mud.mapping.custom.host"))) // priority to the new one
            tmsServer = parameters.value(QStringLiteral("mud.mapping.custom.host")).toString();

        QString mapCopyright;
        QString dataCopyright;
        if (parameters.contains(QStringLiteral("mud.mapping.custom.mapcopyright")))
            mapCopyright = parameters.value(QStringLiteral("mud.mapping.custom.mapcopyright")).toString();
        if (parameters.contains(QStringLiteral("mud.mapping.custom.datacopyright")))
            dataCopyright = parameters.value(QStringLiteral("mud.mapping.custom.datacopyright")).toString();

        if (parameters.contains(QStringLiteral("mud.mapping.copyright")))
            m_customCopyright = parameters.value(QStringLiteral("mud.mapping.copyright")).toString();
    ...
    if (parameters.contains(QStringLiteral("mud.mapping.providersrepository.disabled")))
        disableRedirection = parameters.value(QStringLiteral("mud.mapping.providersrepository.disabled")).toBool();
    ...
    /* TILE CACHE */
    if (parameters.contains(QStringLiteral("mud.mapping.offline.directory")))
        m_offlineDirectory = parameters.value(QStringLiteral("mud.mapping.offline.directory")).toString();
     ...
     */
    if (parameters.contains(QStringLiteral("mud.mapping.cache.disk.cost_strategy"))) {
        QString cacheStrategy = parameters.value(QStringLiteral("mud.mapping.cache.disk.cost_strategy")).toString().toLower();
    ...
    if (parameters.contains(QStringLiteral("mud.mapping.cache.disk.size"))) {
        bool ok = false;
        int cacheSize = parameters.value(QStringLiteral("mud.mapping.cache.disk.size")).toString().toInt(&ok);
        if (ok)
     ...
     */
    if (parameters.contains(QStringLiteral("mud.mapping.cache.memory.cost_strategy"))) {
        QString cacheStrategy = parameters.value(QStringLiteral("mud.mapping.cache.memory.cost_strategy")).toString().toLower();
        if (cacheStrategy == QLatin1String("bytesize"))
    ...
    if (parameters.contains(QStringLiteral("mud.mapping.cache.memory.size"))) {
        bool ok = false;
        int cacheSize = parameters.value(QStringLiteral("mud.mapping.cache.memory.size")).toString().toInt(&ok);
        if (ok)
     ...
     */
    if (parameters.contains(QStringLiteral("mud.mapping.cache.texture.cost_strategy"))) {
        QString cacheStrategy = parameters.value(QStringLiteral("mud.mapping.cache.texture.cost_strategy")).toString().toLower();
        if (cacheStrategy == QLatin1String("bytesize"))
    ...
    if (parameters.contains(QStringLiteral("mud.mapping.cache.texture.size"))) {
        bool ok = false;
        int cacheSize = parameters.value(QStringLiteral("mud.mapping.cache.texture.size")).toString().toInt(&ok);
        if (ok)
    ...
    /* PREFETCHING */
    if (parameters.contains(QStringLiteral("mud.mapping.prefetching_style"))) {
        const QString prefetchingMode = parameters.value(QStringLiteral("mud.mapping.prefetching_style")).toString();
